...
| **Motivação** | O grupo utilizou a IA para organizar e estruturar as informações coletadas na visita à oficina (entrevista com o proprietário, fotos, modelo de ordem de serviço) dentro do formato exigido pelo roteiro da disciplina, e para não perder tempo formatando manualmente cada seção do README. |
| **Prompt(s) utilizados** | 1) O grupo colou o roteiro/análise da Entrega 1 e pediu sugestão de como organizar o trabalho. 2) O grupo informou os dados levantados na visita à Oficina Tricai (nome, tipo, tamanho, endereço, contato, problema operacional e descrição do funcionamento — cadastro, atendimento, pagamento, agendamento, estoque) e pediu para estruturar isso. 3) O grupo enviou fotos da visita (ordem de serviço física, equipe na oficina, letreiro externo, estoque de peças) e o arquivo oficial `00-c_Esqueleto_Entrega_1.md`, pedindo para montar o README seguindo exatamente essa estrutura. 4) O grupo pediu para adicionar PIX e parcelamento em 4x às formas de pagamento e para gerar o DER e o fluxograma como imagens. |
...
| **Trechos rejeitados ou corrigidos** | O modelo inicial de formas de pagamento gerado pela IA foi baseado apenas no talão impresso da Ordem de Serviço (dinheiro, débito, crédito, cheque, conta assinada) e não incluía PIX nem parcelamento. O grupo corrigiu esse ponto após confirmar com o proprietário que essas opções também são praticadas, e a IA foi usada novamente para propagar a correção em todas as seções do README (fluxo de processo, requisitos, regras de negócio e dicionário de dados). |
